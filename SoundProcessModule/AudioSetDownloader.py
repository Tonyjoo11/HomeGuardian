...
class AudioSetDownloader:
    ...
    def load_data(self):
        ...
        fire_alarm_data = data[data['positive_labels'].str.contains(self.fire_alarm_label)]
        non_fire_alarm_data = data[~data['positive_labels'].str.contains(self.fire_alarm_label)]
        return fire_alarm_data, non_fire_alarm_data
    ...
    def download_random_non_fire_alarm_clips(self, num_clips=50):
        # 화재 경보가 아닌 데이터 로드
        _, non_fire_alarm_data = self.load_data()
...
def main():
    ...
    # 화재 경보 데이터를 다운로드합니다.
    downloader.download_fire_alarm_clips()

    # 50개의 랜덤한 non-fire-alarm 데이터를 다운로드합니다.
    downloader.download_random_non_fire_alarm_clips(num_clips=50)
